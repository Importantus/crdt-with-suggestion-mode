...
export const collabSync = ViewPlugin.fromClass(
  class {
    view: EditorView;
    docContent: TrackChangesDocument["content"];
    ...
    update(update: ViewUpdate) {
      const isAnnotation =
        this.view.state.field(dynamicFlagsField).annotationMode;
        ...
        for (const effect of tr.effects) {
          if (effect.is(crdtUpdateEffect)) {
            ...
            if (crdtUpdate.type === "insert" && crdtUpdate.text) {
              ...
              this.config.doc.content.delete(
                crdtUpdate.from,
                length,
                isAnnotation
              );
        ...
        this.docContent.on("Insert", (event) => {
          setTimeout(() => {
            this.view.dispatch({
              ...
              selection: event.meta.isLocalOp
        ...
        this.docContent.on("Delete", (event) => {
          setTimeout(() => {
            this.view.dispatch({
              changes: {
                from: event.index,
                to: event.index + event.values.length,
              },
              selection: event.meta.isLocalOp
                ? { anchor: event.index }
                : undefined,
