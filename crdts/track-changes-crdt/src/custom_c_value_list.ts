/**
 * This file is copied from the collabs library but modified to include a range deletion
 */

import { CTotalOrder, LocalList } from "@collabs/collabs";
import {
...
export interface ValueListInsertMessage {
  /** The created positions' waypoint's counter. (required uint32) */
  counter: number;

  /**
   * The first created position's valueIndex.
   * (optional uint32, default = 0 in proto; undefined in JS/TS means "not set")
   */
  valueIndex?: number;

  /**
   * `oneof data` in proto -> modelliert als discriminated union.
   * Only one of these should be present.
   */
  data?:
    | { kind: "value"; value: Uint8Array }
    | { kind: "valueArray"; valueArray: Uint8Array };
}

/**
 * ValueListMessage has a proto `oneof op` with either `insert` or `delete`.
 * Modelliert als discriminated union so lässt sich sicher damit arbeiten.
 */
export type ValueListMessage =
  | { op: "insert"; insert: ValueListInsertMessage }
  | { op: "delete"; delete: string }
  | { op: "deleteRange"; deleteRange: { from: string; to: string } };
 ...
 */
export class CustomCValueList<T> extends AbstractList_CObject<T, [T]> {
  /**
  ...
  private readonly valueListSerializer: Serializer<ValueListMessage> =
    DefaultSerializer.getInstance<ValueListMessage>();
  ...
  private altReceivePrimitive(message: Uint8Array, meta: MessageMeta): void {
    const decoded = this.valueListSerializer.deserialize(message);
    switch (decoded.op) {
      case "insert": {
        ...
        const values =
          insert.data?.kind === "value"
            ...
            : this.valueArraySerializer.deserialize(
                insert.data?.valueArray ?? new Uint8Array()
              );
      ...
      case "deleteRange": {
        ...
        if (fromIndex !== -1 && toIndex !== -1 && fromIndex <= toIndex) {
          ...
          for (let i = toIndex; i >= fromIndex; i--) {
            const position = this.list.getPosition(i);
            deletedValues.push(this.list.getByPosition(position)!);
      ...
      default:
        throw new Error(`Unknown decoded.op: ${decoded}`);
    }
  }
  ...
  insert(index: number, ...values: T[]): T | undefined {
    ...
    const insertMessage: ValueListInsertMessage = {
      counter: waypoint.counter,
      valueIndex: valueIndex === 0 ? undefined : valueIndex,
      data:
        values.length === 1
          ? { kind: "value", value: this.valueSerializer.serialize(values[0]) }
          : {
              kind: "valueArray",
              valueArray: this.valueArraySerializer.serialize(values),
            },
    };
    this.messenger.sendMessage(
      this.valueListSerializer.serialize({
        op: "insert",
  ...
  delete(index: number, count = 1): void {
    ...
    for (let i = index + count - 1; i >= index; i--) {
      this.messenger.sendMessage(
        this.valueListSerializer.serialize({
          op: "delete",
          delete: this.list.getPosition(i),
        })
      );
    }
  }
  ...
  deleteRange(from: Position, to: Position): void {
    if (!this.list.hasPosition(from) || !this.list.hasPosition(to)) {
      throw new Error(
        `Invalid range: from ${from} or to ${to} not in list positions.`
      );
    }
