...
import { getUserColorFallback } from "./utils/color";
...
export interface TrackChangesOptions {
  ...
  /**
   * Optional function to get a user color based on their ID.
   * This can be used to visually distinguish users in the collaborative editor.
   * If not provided, a default color generation function will be used.
   */
...
export class TrackChangesConfig {
  ...
  constructor(options: Partial<TrackChangesOptions> = {}) {
    ...
    this.userId =
      options.userId ?? `user-${Math.random().toString(36).substring(2, 9)}`;
    this.getUserColor = options.getUserColor ?? getUserColorFallback;
  }
 ...
 */
export const trackChangesFacet = Facet.define<
  TrackChangesConfig,
  TrackChangesConfig
>({
