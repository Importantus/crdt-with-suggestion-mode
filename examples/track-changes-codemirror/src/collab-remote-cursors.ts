  ...
  class {
    ...
    constructor(view: EditorView) {
      ...
      const presenceListener = () => {
        const states = Array.from(config.presence!.entries()).map(
          (p) => p[1] as PresenceState
        );
        setTimeout(() => {
      ...
      setTimeout(() => {
        view.dispatch({
          effects: setRemoteStates.of(
            Array.from(config.presence!.entries()).map(
              (p) => p[1] as PresenceState
            )
          ),
        });
    ...
    buildDecorations(
      ...
      for (const state of states) {
        ...
        const anchorPos = Cursors.toIndex(
          state.selection.anchor,
          config.doc.content
        );
        const headPos = Cursors.toIndex(
